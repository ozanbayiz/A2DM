...
class MotionVAE(nn.Module):
    def __init__(
        ...
        nn.init.xavier_normal_(self.to_mu.weight, gain=0.01)
        nn.init.xavier_normal_(self.to_logvar.weight, gain=0.01)
    ...
    def _get_sinusoidal_embedding(self, length, dim):
    ...
    def update_kl_weight(self, current_epoch, total_epochs):
        """Update KL weight using cyclical annealing schedule"""
        cycle = total_epochs // 4
        ratio = (current_epoch % cycle) / cycle
        self.kld_weight = self.min_kl_weight + (self.max_kl_weight - self.min_kl_weight) * (1 - math.cos(math.pi * ratio)) / 2
